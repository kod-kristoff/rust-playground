...
fn main() -> Result<(), MyAppError> {
    let mapper: HashMap<char, char> = "ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz".chars().zip(
...
impl Error for MyAppError {
    ...
    fn source(&self) -> Option<&(dyn Error + 'static)> {
        match self {
            MyAppError::IOError(e) => e.source(),
            MyAppError::DecodeUtf8Error(e) => e.source(),
        }
    }
}
...
impl std::fmt::Display for DecodeUtf8Error {
    fn fmt(&self, fmt: &mut std::fmt::Formatter) -> std::fmt::Result {
        match self {
            DecodeUtf8Error::InvalidUtf8Codepoint => write!(fmt, "Invalid UTF-8 codepoint"),
        }
    }
}
